...
class ReactExoplayerView extends FrameLayout implements
    ...
    private final Handler progressHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SHOW_PROGRESS:
                    if (player != null) {
                        Integer playBackState = player.getPlaybackState();
    ...
    public double getPositionInFirstPeriodMsForCurrentWindow(long currentPosition) {
        ...
        Integer currentWindowIndex = player.getCurrentWindowIndex();
...
public boolean shouldContinueLoading(long playbackPositionUs, long bufferedDurationUs, float playbackSpeed) {
  ...
  private void initializePlayerSource(ReactExoplayerView self, DrmSessionManager drmSessionManager) {
    ...
    private DrmSessionManager buildDrmSessionManager(UUID uuid,
                                                     String licenseUrl, String[] keyRequestPropertiesArray, int retryCount) throws UnsupportedDrmException {
        if (Util.SDK_INT < 18) {
            return null;
        }
        try {
    ...
    private MediaSource buildMediaSource(Uri uri, String overrideExtension, DrmSessionManager drmSessionManager) {
        ...
        drmProvider = mediaItem1 -> drmSessionManager;
        } else {
        drmProvider = new DefaultDrmSessionManagerProvider(); // ✅ fallback to avoid crash
        }
    ...
    public void onPositionDiscontinuity(Player.PositionInfo oldPosition, Player.PositionInfo newPosition,int reason) {
        ...
        Integer repeatMode = player.getRepeatMode();
        if (repeatMode != null && reason == Player.DISCONTINUITY_REASON_AUTO_TRANSITION
    ...
    public void onPlayerError(PlaybackException e) {
        ...
        String errorString = "ExoPlaybackException type : " + PlaybackException.getErrorCodeName(e.errorCode);
        ...
        boolean needsReInitialization = false;
            // if (e.type == PlaybackException.TYPE_RENDERER) {
    ...
    public void setSelectedTrack(int trackType, String type, Dynamic value) {
        ...
        int[] tracksArray = new int[tracks.size()];
        for (int i = 0; i < tracks.size(); i++) {
            tracksArray[i] = tracks.get(i);
        }

        // Build and apply SelectionOverride
        DefaultTrackSelector.SelectionOverride selectionOverride =
        new DefaultTrackSelector.SelectionOverride(groupIndex, tracksArray);

        parametersBuilder.setSelectionOverride(rendererIndex, groups, selectionOverride);
